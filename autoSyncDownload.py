...
def doScp(srcPath,cnf):
    ...
    try:
        ...
        dstFile = os.path.relpath(srcPath,cnf.currentDir)
        srcFile = posixpath.join(cnf.remoteDir,dstFile.replace(os.path.sep, posixpath.sep))
        if not os.path.exists(cnf.currentDir):
            print(f"mkdir{cnf.currentDir}")
            os.mkdir(cnf.currentDir)
